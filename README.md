...
Clerk / DATABASE_URL を設定しなければデモモードで動作します。
...
| `R2_ACCOUNT_ID` | 将来 | Cloudflare R2 アカウントID |
| `R2_ACCESS_KEY_ID` | 将来 | R2 アクセスキー |
| `R2_SECRET_ACCESS_KEY` | 将来 | R2 シークレットキー |
| `R2_BUCKET` | 将来 | R2 バケット名 |
| `RESEND_API_KEY` | 将来 | Resend メール送信キー |
